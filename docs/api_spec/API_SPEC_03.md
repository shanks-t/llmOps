...
This document defines the public API for PRD_03: LLM-as-a-Judge Evaluator Templates. The SDK provides platform-namespaced evaluation capabilities through `llmops.<platform>.evals`, enabling users to create, register, and use LLM-based evaluators.

**Design Principles:**
- Evaluation is a platform capability accessed via `llmops.<platform>.evals`
...
```
llmops/
├── __init__.py              # Package root (unchanged from PRD_01)
├── arize.py                 # Platform module with __getattr__ for 'evals'
...
The evaluation module is accessed via the platform namespace.

**Access Pattern:**
```python
import llmops

# Access Arize evals (lazy-loaded)
evaluator = llmops.arize.evals.faithfulness(llm=llm)
```

**Alternate Import:**
```python
from llmops.evals import arize as evals

evaluator = evals.faithfulness(llm=llm)
```

**Module-Level Access:**
```python
import llmops.arize.evals
...
### 3.2 `llmops.arize.evals.faithfulness()`
...
### 3.3 `llmops.arize.evals.create_classifier()`
...
def create_classifier(
    ...
    Example (multi-class without scores):
        >>> sentiment_eval = llmops.arize.evals.create_classifier(
        ...     name="sentiment",
...
### 3.4 `llmops.arize.evals.register()`
...
def register(
    ...
    Example:
        ...
        >>> llmops.arize.evals.register("hallucination_check", faithfulness)
        >>>
        >>> # Later, in another module...
        >>> eval_ref = llmops.arize.evals.get("hallucination_check")
    """
```

---

### 3.5 `llmops.arize.evals.get()`
...
def get(
    ...
    Example:
        ...
        >>> evaluator = llmops.arize.evals.get("tone_check")
        >>> scores = evaluator.evaluate({"output": "Hello!"})

    Example (handling missing registration):
        >>> try:
        ...     evaluator = llmops.arize.evals.get("unknown")
...
def list() -> list[str]:
    ...
    Example:
        >>> import llmops
        >>>
        >>> llmops.arize.evals.register("eval_a", evaluator_a)
        >>> llmops.arize.evals.register("eval_b", evaluator_b)
        >>>
        >>> print(llmops.arize.evals.list())
        ['eval_a', 'eval_b']
    """
```
...
```python
>>> import llmops
>>> llmops.arize.evals.faithfulness(llm=llm)
ImportError: Arize evals requires 'arize-phoenix-evals' package.
Install with: pip install llmops[arize-evals]
```

**Key behaviors:**
- `import llmops` always succeeds
- `import llmops.arize` succeeds (module exists)
- `llmops.arize.evals` triggers lazy load and dependency check
...
**Missing evaluator:**
```python
>>> llmops.arize.evals.get("nonexistent")
KeyError: "Evaluator 'nonexistent' not registered"
```

**Empty choices:**
```python
>>> llmops.arize.evals.create_classifier(
...     name="test",
...     prompt_template="...",
...     llm=llm,
...     choices=[]
...
llmops.arize.evals.get("my_eval")  # KeyError - not registered in this process
```
...
pip install llmops[arize-evals]

# Both telemetry and evaluation
pip install llmops[arize,arize-evals]
...
def setup_evaluators():
    ...
    llmops.arize.evals.register("tone", tone_eval)
    llmops.arize.evals.register("hallucination", llmops.arize.evals.faithfulness(llm))

# service.py
import llmops

def evaluate_response(response: str) -> dict:
    tone_eval = llmops.arize.evals.get("tone")
...
| Module | Purpose |
|--------|---------|
| `llmops.arize.evals` | Arize platform evaluation API |
...
evaluator = llmops.arize.evals.get("my_eval")  # KeyError after restart

# GOOD - Register at startup
def on_startup():
    llmops.arize.evals.register("my_eval", create_my_evaluator())
```

### 10.3 Don't Mix Evaluation and Telemetry Dependencies

```python
# BAD - Assuming evals requires telemetry deps
llmops.arize.instrument(...)  # Not needed for evals!

# GOOD - Evals work independently
llmops.arize.evals.faithfulness(llm=llm)  # Just works
```
